...
import { Search, LogOut, BookDashed } from "lucide-react";
...
interface EmailCheckerProps {
  ...
  fetchMailList: () => Promise<void>;
  isAdmin: boolean;
  emailList: Email[];
}

const EmailChecker = ({ onLogout, onDashboardLayout, fetchMailList, isAdmin, emailList }: EmailCheckerProps) => {
  ...
  return (
    <div className="">
      {/* Header */}
      <div className="flex justify-between items-center w-full bg-[#174866] p-4">
        {/* Left-aligned item */}
        {isAdmin ? (
          <div className="flex mx-4">
            <button
              ...
              title="Dashboard"
            >
              <BookDashed className="text-gray-700 w-4 h-4" />
            </button>
          </div>
        ) : null}
      ...
      {emailList.map((row: Email, blockIndex) => {
        ...
        return (
          <div key={blockIndex} className="flex items-center gap-6 p-4 ">
            ...
            <div className="flex-1 overflow-hidden border-2 border-purple-400 rounded p-2">
              <Swiper
